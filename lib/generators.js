...
export const generateOpinion = () => {
    ...
    const displayEmojis = ['🤔', '😤', '🙃', '😶', '🫠', '💀', '🗿', '👁️', '🌚', '🤖', '👽', '🎭', '🦆', '🐸', '🌵', '🍞', '☁️', '🌀', '⚡', '🔮', '🎲', '🎯', '💭', '📢', '🚀', '🛸', '🌍', '🕳️', '✨', '🌈', '🦝', '🐙', '🦑', '🪼', '🦠', '🍄', '🌻', '🔥', '❄️', '🌊', '⭐', '🌙', '☀️', '🪐', '🌋', '🏔️', '🗻', '🏝️', '🎪', '🎡', '🎢', '🎨', '🎬', '📸', '💿', '📀', '🔧', '⚙️', '🧲', '🔬', '🧪', '🧫', '🧬', '💊', '🩺', '🛡️', '⚔️', '🏹', '🪃', '🎸', '🎺', '🎻', '🥁', '🎮', '🕹️', '👾', '🃏', '🀄', '🎴', '🧿', '🪬', '📿', '💎', '🔑', '🗝️', '🪤', '🧸', '🪆', '🖼️', '🪞', '🪟', '🛋️', '🪑', '🚪', '🧊', '🫧', '🌫️', '🦴', '👻', '🎃', '🤡', '👺', '👹', '🤠', '🥸', '🧐', '🤓', '😎', '🥶', '🥵', '🤯', '🫨', '😵‍💫', '🙄', '😏', '😈', '🦊', '🐺', '🦁', '🐯', '🐮', '🐷', '🐵', '🐔', '🐧', '🦉', '🦅', '🐝', '🐛', '🦋', '🐌', '🐚'];
...
export const generateReply = () => {
    ...
    const replyEmojis = ['👀', '🤝', '💯', '🔥', '❄️', '🌊', '⭐', '🎪', '🎭', '🃏', '🗣️', '💬', '📣', '🎤', '👏', '🙌', '👊', '✊', '🤙', '👋', '🫡', '🫶', '💅', '🧠', '👁️‍🗨️', '🦻', '👃', '🫀', '🫁', '🦷', '💭', '🗯️', '💢', '💥', '💫', '✨', '⚡', '☄️', '🌟', '🌠'];
    
    const displayNames = [
        'Reply_Bot_' + Math.floor(Math.random() * 999),
        'Witness_' + Math.floor(Math.random() * 9999),
        'Passerby_' + generateRandom(adjectives),
        'Lurker_Prime_' + Math.floor(Math.random() * 99),
        'The_Commenter_' + Math.floor(Math.random() * 999),
        'Voice_From_Void_' + Math.floor(Math.random() * 99),
        'Spectator_' + Math.floor(Math.random() * 9999),
        'Observer_' + generateRandom(nouns),
        'Wanderer_' + Math.floor(Math.random() * 999),
        'Echo_Chamber_' + Math.floor(Math.random() * 99),
        'Thought_Haver_' + Math.floor(Math.random() * 999),
        'Opinion_Enjoyer_' + Math.floor(Math.random() * 99),
        'Void_Resident_' + Math.floor(Math.random() * 9999),
        'Digital_Ghost_' + Math.floor(Math.random() * 999),
        'Pixel_Phantom_' + Math.floor(Math.random() * 99),
        'Data_Wraith_' + Math.floor(Math.random() * 999),
        'Chaos_Witness_' + Math.floor(Math.random() * 99),
        'Timeline_Drifter_' + Math.floor(Math.random() * 999),
        'Frequency_Walker_' + Math.floor(Math.random() * 99),
        'Signal_Seeker_' + Math.floor(Math.random() * 999),
        generateRandom(adjectives) + '_Replier_' + Math.floor(Math.random() * 99),
        'Thread_' + generateRandom(nouns) + '_' + Math.floor(Math.random() * 99),
        'Discourse_' + generateRandom(adjectives) + Math.floor(Math.random() * 99),
        'Take_Enjoyer_' + Math.floor(Math.random() * 9999),
        'Hot_Take_Fan_' + Math.floor(Math.random() * 999),
        'Cold_Take_Critic_' + Math.floor(Math.random() * 99),
        'Room_Temp_Take_' + Math.floor(Math.random() * 999),
        'Certified_Yapper_' + Math.floor(Math.random() * 99),
        'Professional_Liker_' + Math.floor(Math.random() * 999),
        'Amateur_Scroller_' + Math.floor(Math.random() * 99),
        'Infinite_Browser_' + Math.floor(Math.random() * 999),
        'Feed_Consumer_' + Math.floor(Math.random() * 9999),
        'Content_Entity_' + Math.floor(Math.random() * 99),
        'Engagement_Unit_' + Math.floor(Math.random() * 999),
    ];
    ...
    return {
        ...
        displayName: generateRandom(displayNames),
        displayEmoji: generateRandom(replyEmojis),
